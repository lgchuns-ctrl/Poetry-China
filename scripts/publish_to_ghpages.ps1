param(
  ...
  [switch]$NoCredentialHelper
)

$ErrorActionPreference = "Stop"
$tmp = $null
$Ghc = @()
if ($NoCredentialHelper) { $Ghc = @("-c", "credential.helper=") }
...
try {
  ...
  git @Ghc ls-remote --exit-code origin gh-pages 2>$null
  $remoteExists = ($LASTEXITCODE -eq 0)
  if ($remoteExists) {
    git @Ghc fetch origin gh-pages:refs/remotes/origin/gh-pages
    Assert-GitOk "git fetch 失败，请检查代理与网络"
  } else {
    Write-Host "远端暂无 gh-pages，跳过 fetch"
  }

  git show-ref --verify --quiet refs/remotes/origin/gh-pages
  $remoteTrackExists = ($LASTEXITCODE -eq 0)
